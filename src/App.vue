...
const cameraTopicName = ref("/camera/image_raw/compressed");
let ros = null;
    ...
    <div class="app-container">
        <header class="app-header">
            ...
            <form class="connection-form" @submit="handleSubmit">
                <input v-model="wsUrl" class="url-input" :disabled="isConnecting || isConnected"
                    placeholder="ws://localhost:9090" autocomplete="off" />
                <button type="button" class="btn btn-test" :disabled="!wsUrl.trim()" @click="testConnection"
                    title="测试 WebSocket 连接（不建立实际连接）">
                    🧪 测试
                </button>
                <button type="submit" class="btn btn-primary" :disabled="!wsUrl.trim() || isConnecting || isConnected">
                    连接
                </button>
                <button type="button" class="btn btn-secondary" :disabled="!isConnected && status !== 'error'"
                    @click="disconnect">
                    断开
                </button>
                <button type="button" class="btn btn-secondary" :disabled="!isConnected" @click="getAvailableTopics"
                    title="查询 ROS 系统中所有可用话题">
                    📋 查询话题
                </button>
                <button type="button" class="btn btn-secondary" :disabled="!isConnected"
                    @click="() => getTopicType('/camera/image_raw/compressed')" title="查询摄像头话题的消息类型">
                    📷 查询摄像头
                </button>
                <button type="button" class="btn btn-test" :disabled="!isConnected"
                    @click="() => trySubscribeWithDifferentTypes('/camera/image_raw/compressed')" title="尝试不同的消息类型自动订阅">
                    🔄 尝试订阅
                </button>
            </form>
        </header>
        ...
        <div class="main-layout">
            <section class="view-section">
                <div class="panel-tabs">
                    <button v-for="panel in panelOptions" :key="panel.id" type="button" class="panel-tab"
                        :class="{ active: activePanel === panel.id }" @click="switchPanel(panel.id)">
                        {{ panel.label }}
                    </button>
                </div>
                ...
                <div class="view-stage">
                    <View3D v-if="activePanel === 'navigation'" :ros="ros" :connected="isConnected" />
                    <CameraViewer v-else-if="activePanel === 'camera'" :ros="ros" :connected="isConnected"
                        :topic-name="cameraTopicName" />
                    <MapEditor ref="mapEditorRef" v-else-if="activePanel === 'map-editor'" :ros="ros"
                        :connected="isConnected" />
                    <div v-if="!isConnected && activePanel !== 'map-editor'" class="view-overlay">
                        <p>请连接 机器狗 以查看实时数据</p>
                    </div>
                </div>

                <div class="topics-footer" :class="{ collapsed: topicsCollapsed }">
                    <button type="button" class="topics-toggle" @click="topicsCollapsed = !topicsCollapsed">
                    ...
                    <div v-show="!topicsCollapsed" class="topic-summary" role="list">
                        <div v-for="topic in activeTopics" :key="topic.topic" class="topic-row" role="listitem">
            ...
            <aside class="publisher-section">
                <template v-if="activePanel === 'map-editor' && mapEditorRef">
                    ...
                    <div class="map-tools">
                        <div class="tool-group">
                            ...
                            <select v-model="mapEditorRef.selectedMap" class="tool-select"
                                :disabled="mapEditorRef.listLoading">
                                <option value="" disabled>选择地图</option>
                                <option v-for="map in mapEditorRef.mapsList" :key="map.name" :value="map.name">
                                    {{ map.name }}
                                </option>
                            </select>
                            <button type="button" class="tool-btn secondary" @click="mapEditorRef.fetchMapsList"
                                :disabled="mapEditorRef.listLoading">
                        ...
                        <div class="tool-group">
                            <button type="button" class="tool-btn secondary" @click="mapEditorRef.triggerFileInput"
                                :disabled="mapEditorRef.uploading">
                                {{ mapEditorRef.uploading ? "上传中…" : "上传本地PGM" }}
                            </button>
                            <button type="button" class="tool-btn secondary"
                                :disabled="!mapEditorRef.selectedMap || mapEditorRef.mapLoading" @click="
                        ...
                        <div class="tool-group">
                            <button type="button" class="tool-btn secondary" :disabled="!mapEditorRef.isLoaded"
                        ...
                        <div class="tool-group">
                            ...
                            <input v-model="mapEditorRef.saveAsName" type="text" class="tool-input"
                                placeholder="edited_map.pgm" />
                        </div>

                        <div class="tool-group">
                            <button type="button" class="tool-btn primary"
                                :disabled="!mapEditorRef.isLoaded || mapEditorRef.saving"
                                @click="mapEditorRef.saveToServer">
                                {{ mapEditorRef.saving ? "保存中…" : "保存到服务器" }}
                            </button>
                            <button type="button" class="tool-btn accent" :disabled="!mapEditorRef.isLoaded ||
                ...
                <template v-else>
                    ...
                    <div class="publish-list">
                        <button v-for="preset in activePublishers" :key="preset.topic" type="button"
                            class="publish-button" :disabled="!isConnected || isPresetPublishing(preset)"
